...
def plotar (*args:list,**kwargs):
    ...
    """
    parametros{\n
        args: m listas de pares x,y neste formato: [[x1,x2,...,xn],[y1,y2,...,yn]]\n
        }\n
    kwargs {\n
        escala_x = float (0.1 por padrão)\n
        escala_y = float (0.1 por padrão)\n
        nome_funcoes = [str nome1, str nome2,...,str nome_m]\n
        nome_arquivo = str\n
        linha_espessura = float (1 por padrão)\n
        }
    
    exemplo de uso:\n
    par1 = [[1,2,3,4],[2,8,15,17]]\n
    par2 = [[1,2],[0,0]]
    
    nome_das_funcoes = ["funcao1","funcao2"]\n
    plotar (par1 , par2, nomes = nome_das_fucoes)
    ...
    nomes_das_funcoes=kwargs.get ('nome_funcoes',list (range(len(args))))
    ...
    for numero_do_grafico in range(len (args)):
        eixos.plot (args[numero_do_grafico][0][:-1],args[numero_do_grafico][1][:-1],scalex=escalax,scaley=escalay,linewidth=linha_espessura,label=nomes_das_funcoes[numero_do_grafico])
...
def grafico_duplo (conjunto_1:list,conjunto_2:list,**kwargs):
    ...
    nomes_das_funcoes_1=kwargs.get ('nome_funcoes_1',list (range(1,len(conjunto_1))))
    nomes_das_funcoes_2=kwargs.get ('nome_funcoes_2',list (range(1,len(conjunto_2))))
    ...
    for iterador_1 in range (len(conjunto_1)):
        eixo1.plot (conjunto_1[iterador_1][0],conjunto_1[iterador_1][1],scalex=escalax,scaley=escalay,linewidth=linha_espessura,label=nomes_das_funcoes_1[iterador_1])
    for iterador_2 in range (len(conjunto_2)):
        eixo2.plot (conjunto_2[iterador_2][0],conjunto_1[iterador_2][1],scalex=escalax,scaley=escalay,linewidth=linha_espessura,label=nomes_das_funcoes_2[iterador_2])
    eixo1.grid()
    eixo1.legend()
