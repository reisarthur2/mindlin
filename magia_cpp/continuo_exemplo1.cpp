...
int main (int argc, char* argv[]) {
    ...
    if (stoi (argv[5]) or stoi (argv[6]) or stoi (argv[7])){
        ...
        if (stoi (argv[6]) or stoi (argv[7])){
            ...
            auto N1 = [a_chapeu,raiz,E](long double y) -> long double {
                long double y_real = y/E - floor(y/E);
                if (y_real<(1.0/2)){
                    return a_chapeu/M_PI/raiz*atan(0.75*tan(y_real*M_PI)/raiz)-y_real;
                }else if (y_real-0.5<0.001){
                    return 0;
                } else {
                    return a_chapeu/M_PI/raiz*atan(0.75*tan(y_real*M_PI)/raiz)-y_real+1;
                }
